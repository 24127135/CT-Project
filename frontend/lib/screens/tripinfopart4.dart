...
class _TripRequestScreenState extends State<TripRequestScreen> {
  ...
  void initState() {
    super.initState();
    _noteController = TextEditingController(text: context.read<TripProvider>().note);
  }
  @override
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            TextField(controller: _noteController, onChanged: (value) => context.read<TripProvider>().setNote(value), maxLines: 6, maxLength: 500, decoration: InputDecoration(hintText: 'Nhập yêu cầu của bạn...', fillColor: const Color(0xFFF1F8E9), filled: true, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: Colors.grey.shade400)))),
            const SizedBox(height: 24),
            if (tripData.selectedInterests.isNotEmpty) ...[const Text('Sở thích đã chọn', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)), const SizedBox(height: 12), Wrap(spacing: 8.0, runSpacing: 8.0, children: tripData.selectedInterests.map((interest) => _buildSelectedChip(context, interest)).toList()), const SizedBox(height: 24)],
            const Text('Gợi ý sở thích', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 12),
            Wrap(spacing: 8.0, runSpacing: 8.0, children: _suggestedInterests.map((interest) { if (tripData.selectedInterests.contains(interest)) return const SizedBox.shrink(); return _buildSuggestionChip(context, interest); }).toList()),
      ...
      bottomNavigationBar: Padding(
        ...
        child: Row(
          children: [
            // Nút Back dưới
            Container(decoration: BoxDecoration(border: Border.all(color: Colors.grey.shade300), borderRadius: BorderRadius.circular(8)), child: IconButton(icon: const Icon(Icons.arrow_back_ios_new, size: 20), onPressed: () => Navigator.pop(context))),
            const SizedBox(width: 16),
            Expanded(
              child: ElevatedButton(
                onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const TripConfirmScreen())),
  ...
  Widget _buildSuggestionChip(BuildContext context, String label) {
    return GestureDetector(onTap: () => context.read<TripProvider>().toggleInterest(label), child: Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10), decoration: BoxDecoration(color: Colors.white, border: Border.all(color: primaryGreen.withOpacity(0.5)), borderRadius: BorderRadius.circular(8)), child: Text('+ $label', style: const TextStyle(color: Colors.black87, fontWeight: FontWeight.w500))));
  }
  Widget _buildSelectedChip(BuildContext context, String label) {
    return GestureDetector(onTap: () => context.read<TripProvider>().toggleInterest(label), child: Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10), decoration: BoxDecoration(color: primaryGreen, borderRadius: BorderRadius.circular(8)), child: Row(mainAxisSize: MainAxisSize.min, children: [Text(label, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), const SizedBox(width: 8), const Icon(Icons.close, color: Colors.white, size: 16)])));
  }
}